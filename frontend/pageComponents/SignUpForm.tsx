...
import { NSFW_WORDS } from "@/lib/nsfw";
...
export default function SignUpForm() {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    ...
    const isNsfw = NSFW_WORDS.some((word) => {
      return (
        username.toLowerCase() === word.toLowerCase().trim() ||
        username.toLowerCase().includes(word.toLowerCase().trim()) ||
        username.toLowerCase().endsWith(word.toLowerCase().trim()) ||
        username.toLowerCase().startsWith(word.toLowerCase().trim())
      );
    });
    if (isNsfw) {
      setError("username", { message: "Contains inappropriate content." });
      return;
    }
    if (username.length > 15) {
      setError("username", { message: "Must be 15 characters or fewer." });
      return;
    }
